// ============================================================
// 全局配置 — CDN 前缀、关键常量
// ============================================================

/** CDN 开关：true = 生产模式用 CDN，false = 测试模式用本地资源 */
export const USE_CDN = import.meta.env.VITE_USE_CDN === 'true'

/** 生产环境 CDN 基础路径 */
const CDN_BASE_URL = 'https://ling-birthday.oss-cn-chengdu.aliyuncs.com'

/** 测试环境本地资源路径（public/ 下的目录） */
const LOCAL_ASSET_PATH = '/test-assets'

/**
 * 根据 CDN 开关拼接资源路径
 * 测试模式：映射到 public/test-assets/，保持原始扩展名
 * 生产模式：映射到 CDN 完整路径
 */
export function asset(path: string): string {
  if (USE_CDN) {
    return `${CDN_BASE_URL}${path}`
  }
  // 去掉 /assets/ 前缀，落到 public/test-assets/ 下
  return `${LOCAL_ASSET_PATH}${path.replace('/assets', '')}`
}

/** 背景音乐路径 */
export function bgmAsset(path: string): string {
  if (USE_CDN) {
    return `${CDN_BASE_URL}${path}`
  }
  return `${LOCAL_ASSET_PATH}${path.replace('/assets', '')}`
}

/** 关键资源清单（优先加载） */
export const CRITICAL_ASSETS = [
  { alias: 'match-body', src: asset('/assets/火柴.png') },
  { alias: 'match-head', src: asset('/assets/火柴.png') },
  { alias: 'match-flame', src: asset('/assets/火柴火焰.png') },
  { alias: 'matchbox', src: asset('/assets/火柴盒.png') },
  { alias: 'match-bg', src: asset('/assets/火柴背景.png') },
  { alias: 'fire-particle', src: asset('/assets/particle-smoke.png') },
  { alias: 'room-dark', src: asset('/assets/房间暗.png') },
  { alias: 'room-bright', src: asset('/assets/房间亮.png') },
  { alias: 'card-bg', src: asset('/assets/贺卡.png') },
  { alias: 'card-mascot', src: asset('/assets/贺卡小人.png') },
]

// ============================================================
// 响应式断点
// ============================================================

/** 移动端断点 (px)，viewport 宽度小于此值视为手机 */
export const MOBILE_BREAKPOINT = 768

/** 检测当前是否为移动端 */
export function isMobile(): boolean {
  return window.innerWidth < MOBILE_BREAKPOINT
}

// ============================================================
// 火柴交互配置（桌面端 / 移动端 两套独立配置）
// getMatchConfig() 会根据屏幕宽度自动选择
// 每个字段的注释见 MATCH_CONFIG，MOBILE_MATCH_CONFIG 字段含义完全相同
// ============================================================

/** 火柴交互配置（桌面端 ≥768px） */
export const MATCH_CONFIG = {
  // ---- 划燃触发 ----
  /** 触发点燃的最低滑动速度 (px/帧)，低于此值不会点燃 */
  minSpeed: 10,
  /** 触发点燃的最低滑动总距离 (px)，低于此值不会点燃 */
  minDistance: 80,
  /** 火柴头上方占火柴总高度的比例，用于碰撞检测时定位火柴头 */
  matchHeadRatio: 0.1,
  /** 火柴盒"磷片"区域占火柴盒高度的比例（底部算起），火柴头在此区域内才算有效擦划 */
  phosphorRatio: 0.2,

  // ---- 火柴盒 ----
  /** 火柴盒纹理缩放倍数（固定值，不随屏幕宽度变化） */
  matchboxScale: 1.0,
  /** 火柴盒水平位置（屏幕宽度比例，0=最左 1=最右） */
  matchboxX: 0.45,
  /** 火柴盒垂直位置（屏幕高度比例，0=最顶 1=最底） */
  matchboxY: 0.5,
  /** 火柴盒旋转角度（弧度，负数=逆时针倾斜） */
  matchboxRotation: -1,

  // ---- 火柴本体 ----
  /** 火柴相对于火柴盒中心的水平偏移（倍火柴盒纹理宽度 × 缩放，正=右） */
  matchOffsetX: 1,
  /** 火柴相对于火柴盒中心的垂直偏移（倍火柴盒纹理高度 × 缩放，负=上） */
  matchOffsetY: 0.5,
  /** 火柴身体纹理缩放倍数 */
  matchScale: 0.9,
  /** 火柴初始旋转角度（弧度，负数=逆时针倾斜） */
  matchRotation: -0.5,

  // ---- 火焰 ----
  /** 火焰相对于火柴头顶部的水平偏移 (px) */
  flameOffsetX: 0,
  /** 火焰相对于火柴头顶部的垂直偏移 (px，负=上移) */
  flameOffsetY: 5,
  /** 火焰精灵缩放倍数 */
  flameScale: 0.8,

  // ---- 提示文字 ----
  /** "滑动火柴" 提示文字的字号 (px) */
  hintFontSize: 40,
  /** "滑动火柴" 提示文字相对于火柴盒中心的 X 偏移 (px，正=右) */
  hintXOffset: 90,
  /** "滑动火柴" 提示文字相对于火柴盒底部中心的 Y 偏移 (px，正=向下) */
  hintYOffset: 120,

  // ---- 粒子 ----
  /** 粒子爆发时飞出的最小距离 (px) */
  burstDistMin: 250,
  /** 粒子爆发时飞出的最大距离 (px) */
  burstDistMax: 700,
  /** 萤火虫距离屏幕边缘的最小边距 (px) */
  fireflyMargin: 60,
  /** 吹灭粒子飞散的最小距离 (px) */
  blowDistMin: 250,
  /** 吹灭粒子飞散的最大距离 (px) */
  blowDistMax: 750,
}

/**
 * 火柴交互配置（移动端 <768px）
 * 字段含义与 MATCH_CONFIG 完全相同，只是数值适配了小屏幕。
 * 直接改这里的值即可调手机端效果，不影响桌面端。
 */
export const MOBILE_MATCH_CONFIG = {
  // ---- 划燃触发 ----
  /** 触发点燃的最低滑动速度 (px/帧) — 手机端降低阈值，更容易划燃 */
  minSpeed: 10,
  /** 触发点燃的最低滑动总距离 (px) — 手机屏小，降低距离要求 */
  minDistance: 30,
  matchHeadRatio: 0.1,
  phosphorRatio: 0.2,

  // ---- 火柴盒 ----
  /** 火柴盒缩放 — 手机端缩到和火柴匹配 */
  matchboxScale: 0.65,
  matchboxX: 0.4,
  matchboxY: 0.5,
  matchboxRotation: -1,

  // ---- 火柴本体 ----
  matchOffsetX: 1,
  matchOffsetY: 0.6,
  /** 火柴缩放 — 手机端缩到和火柴盒匹配 */
  matchScale: 0.6,
  matchRotation: -0.5,

  // ---- 火焰 ----
  /** 火焰相对于火柴头的 X 偏移 (px) — 手机端可能需要微调 */
  flameOffsetX: -25,
  /** 火焰相对于火柴头的 Y 偏移 (px，负=上移) */
  flameOffsetY: -45,
  /** 火焰缩放 — 随火柴等比缩小 */
  flameScale: 0.7,

  // ---- 提示文字 ----
  /** 提示文字字号 — 手机屏小，字号略减但仍可读 */
  hintFontSize: 30,
  /** 提示文字 X 偏移 */
  hintXOffset: 45,
  /** 提示文字 Y 偏移 — 手机端离火柴盒更近 */
  hintYOffset: 120,

  // ---- 粒子 ----
  /** 粒子爆发距离 — 手机屏小，粒子飞出太远会看不见 */
  burstDistMin: 100,
  burstDistMax: 250,
  /** 萤火虫边距 — 手机屏窄，缩小边距 */
  fireflyMargin: 20,
  /** 吹灭粒子距离 — 手机端缩到屏幕内可见 */
  blowDistMin: 100,
  blowDistMax: 250,
}

/** 根据当前屏幕宽度自动选择桌面/移动端火柴配置 */
export function getMatchConfig() {
  return isMobile() ? MOBILE_MATCH_CONFIG : MATCH_CONFIG
}

/** 粒子配置 */
export const PARTICLE_CONFIG = {
  /** Stage 2 萤火虫粒子数量 */
  fireflyCount: 25,
  /** 粒子大小范围 */
  minSize: 8,
  maxSize: 22,
  /** 萤火虫漂浮幅度 */
  floatAmplitude: 50,
  /** 收集飞行时长 (秒) */
  collectDuration: 1.0,
  /** 蜡烛/收集目标水平位置 (屏幕宽度比例, 0=左 1=右) */
  candleX: 0.5,
  /** 收集目标垂直位置 (屏幕高度比例, 0=顶 1=底) — 距底部 1/4 */
  candleY: 0.75,
}

/** 贺卡三宫格 + 排版配置 */
export const CARD_CONFIG = {
  /** 贺卡信封图（三宫格背景） */
  cardBgUrl: asset('/assets/贺卡.png'),
  /** 贺卡小人图 URL */
  mascotUrl: asset('/assets/贺卡小人.png'),
  /** 图片原始尺寸 */
  imageWidth: 1657,
  imageHeight: 1117,
  /** 上/下裁切（图片像素，不拉伸） */
  topSlice: 200,
  bottomSlice: 200,
}
export const BLOW_CONFIG = {
  /** 分贝阈值 (RMS, 0~1)，越小越灵敏 */
  dbThreshold: 0.03,
  /** 持续超过阈值的时间 (ms) */
  sustainDuration: 150,
  /** 麦克风超时 (ms) — 超过则显示降级按钮 */
  micTimeout: 3000,
}

/** 音频全局配置 */
export const AUDIO_CONFIG = {
  /** 背景音乐音量 (0~1) */
  bgmVolume: 0.02,
  /** 播放语音/视频时 BGM 压低目标音量 (0~1) */
  duckBgmVolume: 0.008,
  /** 音效音量 (0~1) */
  sfxVolume: 0.5,
  /** 语音音量 (0~1) */
  voiceVolume: 0.7,
  /** 全局静音: true = 静音，适合调试阶段快速关闭 */
  muted: false,
}

...
export class CandleScene extends Container {
  ...
  init(roomTex: Texture, particleTex: Texture, w: number, h: number): void {
    ...
    this.roomBg.texture = roomTex; this.roomBg.anchor.set(0.5); this.roomBg.x = w / 2; this.roomBg.y = h / 2
    const tw = roomTex.width || w, th = roomTex.height || h
    this.roomBg.scale.set(Math.max(w / tw, h / th))
    this.blown = false
  }
  ...
  blowOut(): void {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const p = new FireParticle(this.particleTexture, s); p.alpha = 1; p.zIndex = 10; this.addChild(p); this.particles.push(p)
  ...
  onResize(w: number, h: number): void {
    this.sceneW = w; this.sceneH = h
    this.cfg = getMatchConfig()
    const tw = this.roomBg.texture.width || w, th = this.roomBg.texture.height || h
    this.roomBg.scale.set(Math.max(w / tw, h / th)); this.roomBg.x = w / 2; this.roomBg.y = h / 2
  }

  destroyScene(): void {
    this.analyzer.stop()
    for (const p of this.particles) { gsap.killTweensOf(p); if (p.parent) p.parent.removeChild(p); p.destroy() }
    this.particles = []

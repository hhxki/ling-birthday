...
<template>
  <div ref="canvasContainer" class="fixed inset-0 z-5">
    <Transition name="fade">
      <div v-if="state.micStatus === 'requesting'" class="fixed bottom-15 left-1/2 -translate-x-1/2 z-60 flex items-center gap-3 px-6 py-3 rounded-3xl bg-[#131a26]/75 backdrop-blur-xl border border-[#ffb03a]/30">
        <span class="w-3 h-3 rounded-full bg-[#ffb03a] animate-[micPulse_1s_ease-in-out_infinite]" />
        <p class="m-0 text-[13px] tracking-wider text-white/70">请允许麦克风访问，对着麦克风吹气吧</p>
      </div>
    </Transition>
    <Transition name="fade">
      <button v-if="showFallbackBtn" class="fixed bottom-20 left-1/2 -translate-x-1/2 z-60 flex items-center gap-3 px-8 py-4 rounded-3xl border border-[#ffb03a]/40 bg-[#ffb03a]/12 text-[#ffb03a] text-[15px] cursor-pointer transition-all duration-300 hover:bg-[#ffb03a]/25 hover:shadow-[0_0_32px_rgba(255,176,58,0.25)]" @click="manualBlow">
